...
module.exports = class baixar_drive {
   ...
   */
  static listFiles = async (authClient) => {
  ...
  static baixaArquivo =  async(authClient, fileId, destino, extensao) => {
    const drive = google.drive({ version: "v3", auth: authClient });

    const destStream =  ff.createWriteStream(
      `${__dirname}/${destino}${extensao}`
    );

   const funcao = await drive.files.get(
      { fileId, alt: "media" },
      { responseType: "stream" },
      (err, res) => {
        if (err) {
      ...
      res.data.pipe(destStream);
        ...
        // Fecha o stream de gravação quando o download for concluído
         destStream.on("finish", () => {
          console.log(`File downloaded to ${destino} `);
          // process.exit()
          
          return "sucessooooooooooo"
        });
